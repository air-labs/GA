...
namespace AIRLab.Thornado
{
    public abstract partial class TypeResolver
    {
        ...
        public ADCache TypeAD { get; private set; }
        ...
        public object Create(IEnumerable<KeyValuePair<string, object>> arguments)
        {
            var list = new LogicErrorList(LogicErrorType.Internal);
            var res=Create(arguments, list);
        ...
        public static void TreeRunDown(object obj, Action<FieldAddress, object> rootFirstAction, Action<FieldAddress, object> leafFirstAction)
        {
            TreeRunDown(FieldAddress.Root, obj, rootFirstAction, leafFirstAction);
        }

        public static void CheckConsistancy(object obj, LogicErrorList list)
        {
            TreeRunDown(new FieldAddress(), obj, (a, o) =>
                                                     {
                                                         if (o is ICheckable)
                                                         {
                                                             list.SetAddress(a);
                                                             (o as ICheckable).CheckSelfConsistancy(list);
                                                         }
                                                     }, null);
        }
        #endregion
